...
export async function GET(request: Request) {
	...
	if (secret !== serverConfig.jssEditingSecret) {
		return new NextResponse(JSON.stringify({ message: 'Missing or invalid editing secret' }), {
			status: 401,
		})
	}

	const components = Array.isArray(builderComponents) ? builderComponents : Array.from(builderComponents.keys())
