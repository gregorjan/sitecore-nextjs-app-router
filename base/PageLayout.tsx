...
import type { DictionaryPhrases, LayoutServiceData } from '@sitecore-jss/sitecore-jss-nextjs'
...
export const PageLayout: React.FC<LayoutProps> = ({ layoutData, dictionary }) => {
	...
	const { headerPlaceholders, mainPlaceholders, footerPlaceholders } = Object.entries(route?.placeholders ?? {}).reduce(
		(acc, [placeholderKey, placeholderValue]) => {
			...
			const placeholder = (
				<PlaceholderMetadata key={key} editMode={editMode || null} name={placeholderKey} placeholders={placeholders}>
